...
namespace PDF2XLS
{
    class Program
    {
        ...
        {
            string baseUrl = "https://www.nudelta.pl/api/v1";
            string username = "";
            string password = "";
            ...
            static string GetValFromNode(JsonNode node)
            {
                switch (node)
                {
                    case null:
            ...
            JsonArray rows = tablesNode?["rows"]?.AsArray() ?? new JsonArray();
            JsonArray totals = tablesNode?["total"]?.AsArray() ?? new JsonArray();
